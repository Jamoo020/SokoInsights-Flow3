...
export const Route = createFileRoute("/_member/categories")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse SokoInsights research categories: telecom, banking, consumer research and premium studies.",
      },
      { property: "og:title", content: "Survey Categories | SokoInsights" },
      { property: "og:description", content: "Telecom, banking, research and premium survey categories." },
    ],
  }),
...
function Categories() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-3xl font-extrabold tracking-tight text-ink sm:text-4xl">Explore categories</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {order.map((cat) => {
          ...
          const max = Math.max(...surveys.map((s) => s.maxReward));
          return (
            <section
              key={cat}
            ...
            >
              <div className="relative aspect-[16/7] overflow-hidden bg-secondary">
                ...
                <div className="absolute inset-x-0 bottom-0 p-5">
                  ...
                  <h2 id={`cat-${cat}`} className="mt-2 text-2xl font-extrabold text-primary-foreground">
                    {CATEGORY_LABELS[cat]}
                  </h2>
                  <p className="text-sm font-semibold text-primary-foreground/85">
                    Maximum possible reward: {ksh(max)}
                  </p>
                </div>
              </div>
                  ...
                  return (
                    <li key={s.id}>
                      ...
                      >
                        ...
                        <span className="shrink-0 text-sm font-bold text-accent-foreground">up to {ksh(s.maxReward)}</span>
                        {unlocked ? (
